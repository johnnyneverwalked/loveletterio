...
Simply download the project and use ```npm install package.json```.To start the server use ```npm start``` or with nodemon ```npm start-dev```. The server runs at port 1234.
